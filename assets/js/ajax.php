<script type="text/javascript">
	...
	$(document).on("click", ".detail-dataNasabah", function() {
		var id = $(this).attr("data-id");
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Nasabah/detail'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
			$('#tempat-modal').html(data);
			...
			$('#detail-nasabah').modal('show');
		})
	})

	$(document).on("click", ".detail-dataNasabahSLIK", function() {
		var id = $(this).attr("data-id");
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Nasabah/detail_SLIK'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
			$('#tempat-modal').html(data);
			...
			$('#detail-nasabahslik').modal('show');
		})
	})



	$(document).on('submit', '#form-update-nasabah', function(e){
		var data = $(this).serialize();

		$.ajax({
			method: 'POST',
			url: '<?php echo base_url('Nasabah/prosesUpdate'); ?>',
			data: data
		})
		.done(function(data) {
			var out = jQuery.parseJSON(data);
			...
			if (out.status == 'form') {
				...
				document.getElementById("form-update-nasabah").reset();
				$('#update-nasabah').modal('hide');
				$('.msg').html(out.msg);
				effect_msg();
			}
		})
	...
	$('#tambah-nasabah').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})

	$('#update-nasabah').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})
